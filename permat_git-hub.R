...
extract_last_net <- function(remu_tbl, mykey, years = NULL) {
  ...
  vals <- suppressWarnings(as.numeric(rows$net_pay__))
  vals <- vals[is.finite(vals)]
  if (!length(vals)) {
    vals <- suppressWarnings(as.numeric(rows$pay__))
    vals <- vals[is.finite(vals)]
  }
...
simuler_id <- function(id, generation, taux_cotisation, tables) {
  ...
  tmi_vec <- if (cas_type %in% c(1L, 2L)) mapply(TMI_from_income_dyn, rev = pay_vec, annee = years) else rep(0, N)
